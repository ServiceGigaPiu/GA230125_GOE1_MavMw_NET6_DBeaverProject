...
DROP PROCEDURE IF EXISTS `RecuperaRepository` $$
CREATE DEFINER=`readonly`@`%` PROCEDURE `RecuperaRepository`(

    IN sezione varchar(10), -- 01A / 01B
    IN corsia varchar(10), -- L / R
    IN nScaffale varchar(10), -- 01 / 02 / ... 06
    IN idRepository varchar(10) -- A1 / A2 / A3 -- B1 / B2 / B3
...
begin
    ...
    SET tipoRep = 'SHL';
    SET strRicerca = CONCAT(tipoRep,'.', sezione,'.',corsia,nScaffale,idRepository);
    ...
    select 
        ...
        -- colonna abilitata
         case 
	     	when r.is_disabled = 0 then 'NO'
            when r.is_disabled = 1 then 'SI'
       	 end as abilitata,
