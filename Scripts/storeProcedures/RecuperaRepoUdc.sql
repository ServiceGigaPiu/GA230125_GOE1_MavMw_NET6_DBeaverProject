...
DROP PROCEDURE IF EXISTS `RecuperaRepoUdc` $$
CREATE DEFINER=`readonly`@`%` PROCEDURE `RecuperaRepoUdc`(
...
begin
    ...
    from 
       ...
       select rt.tag from RecordsTemp rt where rt.tag_udc = strRicerca;
END
$$
delimiter ;